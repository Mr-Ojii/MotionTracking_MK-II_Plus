...
//	PIXEL Struct
typedef	struct {
	unsigned char	b, g, r;		//	Pixel(RGB)Data (0～255)
} PIXEL;
...
//	※For de-interlace filter, ycp_edit is not initialized with image data。
//	※De-interlace filter cannot change ycp_edit,ycp_temp,w and h
...
typedef struct {
	//	※Use of get_ycp_source_cache() is preferred
	//
	...
	//	filter_id
	//		 	: Filter ID (0～Total filter count-1)
	//  Return Value	: pointer to Filter Struct (Return NULL when Failed)
	void 		*(*get_filterp)(int filter_id);

	//	※Please use get_ycp_filtering_cache_ex() instead whenever possible
	// Obtains Image Data's pointer of specified Frame
	...
	//	tr      : Transparency of source (0～4096)
	...
	//	※File and format can be different from the one being edited
...
typedef struct {
	...
	int		ex_data_size;		//	size of extended data ( when FILTER_FLAG_EX_DATAが is set)
	...
	//	Return Value		: TRUE for Frame to be saved、FALSE for Frame to be dropped
